...
static int lua_ecs_set(lua_State *L) {
  ...
  if (lua_istable(L, 4)) {
      if (strcmp(type_name, "l_point_t") == 0) {
          ...
          if (lua_objlen(L, 4) == 2) { // Positional array
              lua_rawgeti(L, 4, 1);
              point->x = luaL_optnumber(L, -1, 0);
              lua_pop(L, 1);
              lua_rawgeti(L, 4, 2);
              point->y = luaL_optnumber(L, -1, 0);
              lua_pop(L, 1);
          } else { // Named fields
              lua_getfield(L, 4, "x");
              point->x = luaL_optnumber(L, -1, 0);
          ...
          if (lua_objlen(L, 4) == 2) {
              lua_rawgeti(L, 4, 1);
              vel->vx = luaL_optnumber(L, -1, 0);
              lua_pop(L, 1);
              lua_rawgeti(L, 4, 2);
              vel->vy = luaL_optnumber(L, -1, 0);
              lua_pop(L, 1);
          } else {
              lua_getfield(L, 4, "vx");
              vel->vx = luaL_optnumber(L, -1, 0);
      ...
      if (!flecs_lua_extract_cdata(L, 4, type_name_ptr, type->size, buffer, "ecs_set")) {
          free(buffer);
          return luaL_error(L, "Failed to extract %s cdata", type_name);
      }
  }
...
static int lua_ecs_set_velocity(lua_State *L) {
  ecs_world_t **world_ud = (ecs_world_t **)luaL_checkudata(L, 1, ECS_WORLD_MT);
  ecs_world_t *world = *world_ud;
  ecs_entity_t entity = luaL_checkinteger(L, 2);

  l_velocity_t vel_buffer;
  l_velocity_t *vel = (l_velocity_t *)flecs_lua_extract_cdata(L, 3, "l_velocity_t*", sizeof(l_velocity_t), &vel_buffer, "ecs_set_velocity");
  if (!vel) {
      return luaL_error(L, "Failed to extract l_velocity_t cdata");
  }

  lua_getfield(L, LUA_REGISTRYINDEX, "Lua_Velocity_ID");
  ecs_entity_t comp_id = lua_tointeger(L, -1);
  lua_pop(L, 1);

  printf("Setting l_velocity_t component for entity %llu:\n", (unsigned long long)entity);
  printf("Type: l_velocity_t (size: %zu, alignment: %zu)\n", sizeof(l_velocity_t), ECS_ALIGNOF(l_velocity_t));
  printf("Values: vx = %f, vy = %f\n", vel->vx, vel->vy);

  ecs_set_id(world, entity, comp_id, sizeof(l_velocity_t), vel);
  return 0;
}
...
static int lua_ecs_get_velocity(lua_State *L) {
  ecs_world_t **world_ud = (ecs_world_t **)luaL_checkudata(L, 1, ECS_WORLD_MT);
  ecs_world_t *world = *world_ud;
  ecs_entity_t entity = luaL_checkinteger(L, 2);
  ...
  printf("Getting l_velocity_t component for entity %llu:\n", (unsigned long long)entity);
  printf("Type: l_velocity_t (size: %zu, alignment: %zu)\n", sizeof(l_velocity_t), ECS_ALIGNOF(l_velocity_t));
  printf("Values: vx = %f, vy = %f\n", vel->vx, vel->vy);

  lua_getglobal(L, "ffi");
  lua_getfield(L, -1, "new");
  lua_pushstring(L, "l_velocity_t");
  if (lua_pcall(L, 1, 1, 0) != LUA_OK) {
      printf("FFI new error: %s\n", lua_tostring(L, -1));
      luaL_error(L, "failed to create l_velocity_t cdata");
  }

  lua_getglobal(L, "ffi");
  lua_getfield(L, -1, "copy");
  lua_pushvalue(L, -3);
  lua_pushlightuserdata(L, (void *)vel);
  lua_pushinteger(L, sizeof(l_velocity_t));
  if (lua_pcall(L, 3, 0, 0) != LUA_OK) {
      printf("FFI copy error: %s\n", lua_tostring(L, -1));
      luaL_error(L, "failed to copy l_velocity_t data");
  }
...
static const luaL_Reg flecs_funcs[] = {
    ...
    {"lua_ecs_set_velocity", lua_ecs_set_velocity},
    {"lua_ecs_get", lua_ecs_get},
    {"lua_ecs_get_velocity", lua_ecs_get_velocity},
